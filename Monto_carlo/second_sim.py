...
target = np.array([[338,132,0,1],
				   ...
				   [100,317,0,2],
				   [25,258,0,1]])							# target for A nd B respectively
targt_number = 10
...
robot_B_pos = np.around(np.array([350,0,pi/2,0]), decimals = 3)
...
if __name__ == '__main__':
	...
	for k in range(6):
		...
		while ckr == 0:
			...
			vel_err_X = 0
			vel_err_Y = 0
			if (Time<6.9) or (Time>10 and Time<17) or (Time>17.5 and Time<43) or (Time>48 and Time<72.2) or (Time>75 and Time<90):	
			...
			if kk == 1 and reconf_status == [1,1]:
				...
				if matrix_pos[0]>250 and matrix_pos[1]>110 and matrix_pos[1]<200 and t<=3: 
					pose = pos_list[t]
					t = t + 1
				...
				if (matrix_pos[0]<115) and matrix_pos[1]>110 and t<=13: 
					pose = pos_list[t]
					t = t + 1
				if (matrix_pos[0]<115) and matrix_pos[1]>200 and t<=20: 
					pose = pos_list[t]
					t = t + 1		
