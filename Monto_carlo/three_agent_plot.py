...
target = np.array([[2,70,0],[22,70,0],[30,23.5,0]])								# target for A nd B respectively
targt_number = 3
...
def initialize_local_wrt_global(matrix_pos,kk):						## the position according to the global frame is assigned to the local coordinates
	...
	if kk == 1:
		mat_wrt_global[something[0]][something[1]] = target_loc[0:2] 
...
def local_matrix_target_for_reconf(target_location,x_mat_wrt_global,alpha):
	...
	# 	T[1][0] = T2[0][0]
	# 	T[1][1] = T2[1][0]
	# 	beta.append([T2[0][0],T2[1][0]])	
	# 	for k in range(n):
	# 		if k == 1:
	# 			continue
	# 		else:
	# 			if 3*T[1][0] + T[0][1] == 3*T[k][0] + T[k][1]:
	# 				for l in range(n):
	# 					if l == k:
	# 						continue
	# 					else:	
	...
	# 	T[2][0] = T3[0][0]
	# 	T[2][1] = T3[1][0]
	# 	beta.append([T3[0][0],T3[1][0]])
	# 	for k in range(n):
	# 		if k == 2:
	# 			continue
	# 		else:
	# 			if 3*T[2][0] + T[0][1] == 3*T[k][0] + T[k][1]:
	# 				for l in range(n):
	# 					if l == k:
	# 						continue
	# 					else:	
...
def robot_pos_update_wrt_local(pose,dec):
	...
	for i in range (n):			
		if dec[i] == [0]:
			if pose[i][0]+1 >= 0 and pose[i][0]+1 < 5:
				pose[i] = [pose[i][0]+1, pose[i][1]]
		elif dec[i] == [1]:
			if pose[i][0]-1 >= 0 and pose[i][0]-1 < 5:
				pose[i] = [pose[i][0]-1, pose[i][1]]
		elif dec[i] == [2]:
			if pose[i][1]-1 >= 0 and pose[i][1]-1 < 5:
...
def target_check(target,matrix_pos):
	...
	for t in range (n):
		if target[t][2] == 0:
			if target[t][0] < matrix_pos[0] + 10 and target[t][0] > matrix_pos[0] - 10 and target[t][1] < matrix_pos[1] + 15 and target[t][1] > matrix_pos[1] - 15:
...
def target_status_update():
	xyz = [robot_A_pos,robot_B_pos,robot_C_pos]
	for i in range (targt_number):
		for j in range (n):
			if sqrt((int(target[i][0]) - int(xyz[j][0]))**2 + (int(target[i][1]) - int(xyz[j][1]))**2) <= 1:
...
if __name__ == '__main__':
	...
	for k in range(9):
		...
		ckr = 0
		kk = 0		
		initialize_local_wrt_global(matrix_pos,kk)
		...
		while ckr == 0:
			...
			if kk == 1 and one_time_variable == 0:
			 ...
			 #print(final_local_wrt_global)
				...
				T, something = local_matrix_target_for_reconf(target_capture,final_mat_wrt_global,qwerty)
				#target_nullified = 0
				#print(pose)
				#print(qwerty,"----------------------")
				#kk = 0
			...
			initialize_local_wrt_global(matrix_pos,kk)
			if kk == 1 and reconf_status == [1,1,1] and target_nullified==0:
				#one_time_variable = 1
				if  T == pose:	
					print("true")				
					target_nullified = 1
		...
		###### for plotting purpose
			...
			plt.plot(mat_wrt_global[1][1][0],mat_wrt_global[1][1][1],'bX')
