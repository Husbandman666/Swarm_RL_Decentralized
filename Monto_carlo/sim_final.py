...
target = np.array([[338,132,0],
				   ...
				   [25,198,0],
				   [25,246,0],
				   [100,290,0]])								# target for A nd B respectively
targt_number = 10
...
prev_err_C = [0,0]
targ_rwrd = 5
matrix_target_list = np.array([[300,60],[300,260],[180,260],[180,60],[60,60],[60,300]])
target_points= np.array([[300,132],[300,260],[180,95],[60,250]])

#MATRIX NAVIGATION

def error_calculation(matrix_pos,next_target):									## error calculation for matrix navigation
	global vel_err_X,vel_err_Y,targ
	xyz = copy.deepcopy(matrix_pos)
	#targ = np.array([np.median(next_target[:,0]),np.median(next_target[:,1])])
	targ = [next_target[0],next_target[1]]
	vel_err_X = -np.around((xyz[0] - targ[0]),decimals = 2)					##distance between matrix and target for driving the velocity
	vel_err_Y = -np.around((xyz[1] - targ[1]),decimals = 2)
...
def error_calculation_targ(matrix_pos,next_target):									## error calculation for matrix navigation
	...
	vel_err_X = 0
	vel_err_Y = -np.around((xyz[1] - targ_new[1]),decimals = 2)	
		...
		if 0.0 in P:
			for i in range(len(beta)):
				if beta[i][2] == 0.0:		
					...
					# else:
					mat_wrt_global_A[beta[i][0]][beta[i][1]] = [target_loc[ P.index(0)][ 0 ], target_loc[ P.index(0)][ 1 ]] 			 
		...
		if 1.0 in P:		
			for i in range(len(beta)):
				if beta[i][2] == 1.0:
					...
					# else:
					mat_wrt_global_B[beta[i][0]][beta[i][1]] = [target_loc[ P.index(1)][ 0 ], target_loc[ P.index(1)][ 1 ]] 			 
		if 2.0 in P:		
			for i in range(len(beta)):
				if beta[i][2] == 2.0:
					# if len(target_loc.shape) == 1:
					...
					# else:
					mat_wrt_global_C[beta[i][0]][beta[i][1]] = [target_loc[ P.index(2)][ 0 ], target_loc[ P.index(2)][ 1 ]] 			 
...
def check_if_reached(matrix_pos,tar):										##This funtion is finally aimed towards deciding the final position of the matrix required and if reached or not
	global ckr
	xyz = copy.deepcopy(matrix_pos)
	dist = sqrt((xyz[0] - tar[0])**2 + (xyz[1] - tar[1])**2)
	if dist <= 0.1:
		ckr = 1
		#print("hello")
	else:
...
def error_calculation_ROBOT_A(pos,mat_wrt_global):
	...
	err_A = np.array([vel_err_A_X,vel_err_A_Y])
	return err_A
...
def robot_A_navigation(u_A):
	...
	si = pi
	x = robot_A_pos[0] + u_A[0]*dt
	y = robot_A_pos[1] + u_A[1]*dt
...
def target_check(target,matrix_pos):
	...
	for t in range (number_of_targets):
		if target[t][0] < matrix_pos[0] + 60 and target[t][0] > matrix_pos[0] - 60 and target[t][1] < matrix_pos[1] + 120 and target[t][1] > matrix_pos[1] - 120:
			if target[t][2] == 0:
		...
		while ckr == 0:
			...
			if kk == 1 and one_time_variable == 0:
				...
				T, beta = local_matrix_target_for_reconf(target_capture,final_mat_wrt_global,qwerty)
								...
								#kk = 0
				print(T)
			#T, _ = local_matrix_target_for_reconf(target_capture,final_mat_wrt_global,qwerty)	
			...
			# 	###########
				# 
			robot_pose = [robot_A_pos,robot_B_pos,robot_C_pos]
			check_if_next_step_reached(pose,mat_wrt_global,mat_wrt_global_A,mat_wrt_global_B,mat_wrt_global_C)
			...
			for i in range(number_of_targets):
				if target[i][2] == 0:
					plt.plot(target[i][0],target[i][1],'kx')
			# plt.plot(target[1][0],target[1][1],'kx')
			# plt.plot(target[2][0],target[2][1],'kx')
			# plt.plot(target[3][0],target[3][1],'kx')
			# plt.plot(target[4][0],target[4][1],'kx')
			# plt.plot(target[5][0],target[5][1],'kx')
			# plt.plot(target[6][0],target[6][1],'kx')
			# plt.plot(target[7][0],target[7][1],'kx')
			# plt.plot(target[8][0],target[8][1],'kx')
			# plt.plot(target[9][0],target[9][1],'kx')
