...
robot_A_pos = np.around(np.array([260,0,pi/2]), decimals = 3)

robot_B_pos = np.around(np.array([300,0,pi/2]), decimals = 3)

robot_C_pos = np.around(np.array([340,0,pi/2]), decimals = 3)
...
prev_err_C = [0,0]
targ_rwrd = 5
matrix_target_list = np.array([[300,60],[300,300],[180,300],[180,80],[60,80],[60,300]])
target_points= np.array([[300,132],[300,260],[180,80],[60,246]])
#MATRIX NAVIGATION
...
def target_check(target,matrix_pos):
	...
	for t in range (number_of_targets):
		if target[t][0] < matrix_pos[0] + 60 and target[t][0] > matrix_pos[0] - 60 and target[t][1] < matrix_pos[1] + 60 and target[t][1] > matrix_pos[1] - 60:
			if target[t][2] == 0:
