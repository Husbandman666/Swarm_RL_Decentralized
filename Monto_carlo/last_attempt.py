...
target = np.array([[338,132,0,0],
				   ...
				   [100,300,0,0],
				   [10,266,0,0]])								# target for A nd B respectively
targt_number = 10
...
targ_dash = np.array([[[1,0],[0,1],[1,2]],
					  [[1,0],[0,0],[1,2]],
					  [[1,0],[1,1],[0,2]],
					  [[2,0],[1,0],[0,1]]])

P_dash = np.array([[2.0,1.0,0.0],
				   [1],
				   [2,0,1],
				   [0,2,1]])
...
prev_err_C = [0,0]
targ_rwrd = 5
matrix_target_list = np.array([[300,60],[300,260],[180,260],[180,60],[60,60],[60,340]])
target_points= np.array([[300,132],[300,260],[180,60],[60,266]])
...
# 	if err_C[0] <= 0.05:
# 		if err_C[1] < pi/3:
# 			v = 2*err_C[0] + 0.15*(err_C[0] - prev_err_C[0])
# 			if v > 5:
# 				v = 5
# 		else:
# 			v = 0
...
def check_if_next_step_reached(pose,mat_wrt_global,mat_wrt_global_A,mat_wrt_global_B,mat_wrt_global_C):					 #This function is to decide whether the required posee is reached or not
	...
	A_reached,B_reached,C_reached = 0,0,0
	#for robot A	
...
def target_check(target,matrix_pos):
	...
	for t in range (number_of_targets):
		if target[t][0] < matrix_pos[0] + 60 and target[t][0] > matrix_pos[0] - 60 and target[t][1] < matrix_pos[1] + 120 and target[t][1] > matrix_pos[1] - 80:
			if target[t][2] == 0 and target[t][3] == 0 :
	...
	for i in range (number_of_targets):
		if target[i][2] == 0:
			for j in range (n):	
				...
				if sqrt((robot_pose[j][0] - target[i][0])**2 + (robot_pose[j][1] - target[i][1])**2) < 8:
					print(j)
					target[i][2] = 1
					robot_pose[j][3] = 0
					P.remove(j)		
...
if __name__ == '__main__':
	...
	for k in range(6):
		...
		while ckr == 0:
			...
			if kk == 1 and one_time_variable == 0:
				...
				if T[2] == [1,0]:
					T[2] = [0,2]
				# T = targ_dash[v]
				# beta = [[T[0][0],T[0][1], 0.0], [T[0][0],T[0][1], 1.0], [T[0][0],T[0][1], 2.0]]
				#P = P_dash[v]
				v = v+1
				#print(T)
				#target_nullified = 0
		...
		###### for plotting purpose
			...
			plt.plot(target[0][0],target[0][1],'kx')
