...
import pylab
import numpy as np
from environment_edit_6 import Env
...
if __name__ == "__main__":
    ...
    agent.model.load_weights("./save_model/new_dawn_series3.h5")
    #agent.model.load_weights("./save_model/agent2_new.h5")
    ...
    state = env.reset()
    state1 = np.reshape(state[0], [1, 12])
    state2 = np.reshape(state[1], [1, 12])
    ckr = [0,0]
    for e in range(EPISODES):
        ...
        # if e <= 2000:
        #     action1 = np.random.choice([0,1,2,3])
        #     action2 = np.random.choice([0,1,2,3])
        # else:    
        ...
        next_state1 = np.reshape(next_state[0], [1, 12])
        next_state2 = np.reshape(next_state[1], [1, 12])
            
            #print(reward)
        agent.append_sample(state1, action1, reward[0])
        agent.append_sample(state2, action2, reward[1])
        score += reward[0]
        score2 += reward[1]
        ...
        if e % 100 == 0:
            ...
            agent.model.save_weights("./save_model/new_dawn_series3.h5")
