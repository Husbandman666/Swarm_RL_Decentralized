...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        self.goal = []
        # add image to canvas
        x1, y1 = 3*(UNIT/2), (UNIT/2)
        x2, y2 = (UNIT/2), 3*(UNIT/2)
		
        self.rectangle1 = canvas.create_image(x1, y1, image=self.shapes[0])
    ...
    def reset_reward(self):
        ...
        #self.rewards1.append(self.set_reward([1,1], 2))
        self.rewards1.append(self.set_reward([1,1], 1))
        #self.rewards1.append(self.set_reward([0,1], 1))
        #self.rewards1.append(self.set_reward([1,0], 1,1))
        #self.rewards1.append(self.set_reward([0,1], 1,1))
        
        
        #self.rewards2.append(self.set_reward([0,0], 2)) 
        #self.rewards2.append(self.set_reward([1,0], 1))                       
        self.rewards2.append(self.set_reward([0,0], 1))
    ...
    def set_reward(self, state, reward):
        ...
        location = [self.coords_to_state(self.canvas.coords(self.rectangle1)) , self.coords_to_state(self.canvas.coords(self.rectangle2))]            
    ...
    def reset(self):
        ...
        x, y = self.canvas.coords(self.rectangle1)
        self.canvas.move(self.rectangle1, 3*(UNIT / 2) - x, UNIT / 2 - y)
        
        x1, y1 = self.canvas.coords(self.rectangle2)
        self.canvas.move(self.rectangle2, (UNIT / 2) - x1, 3*(UNIT / 2) - y1)
        #self.canvas.move(self.rectangle1, - 50, - 50)
    ...
    def step(self, action1,action2,target_x,target_y):
        ...
        next_coords = self.move(self.rectangle1, action1 , self.rectangle2 , action2)
        location = [self.coords_to_state(self.canvas.coords(self.rectangle1)) , self.coords_to_state(self.canvas.coords(self.rectangle2))]            
       
        #self.rewards = self.move_rewards(self.rewards)
        #self.rewards2 = self.move_rewards(self.rewards2)
        #if location[0] == location[1]:            
        self.rewards1.append(self.set_reward([location[1][0], location[1][1]], -1))
        self.rewards2.append(self.set_reward([location[0][0], location[0][1]], -1))
        ...
        check1 = self.check_if_reward(self.coords_to_state(next_coords[0]),self.rewards1)
        ...
        reward1 -= 0.1
        reward2 -= 0.1
        self.canvas.tag_raise(self.rectangle1)
    ...
    def get_state(self):
        ...
        location = self.coords_to_state(self.canvas.coords(self.rectangle1))
        location2 = self.coords_to_state(self.canvas.coords(self.rectangle2))
        agent_x = location[0]
        agent_y = location[1]
        ...
        required_order0=[[0,0],[1,0],[1,1],[0,1]]
        required_order1=[[1,0],[1,1],[0,1],[0,0]]
        required_order2=[[1,1],[0,1],[0,0],[1,0]]
        required_order3=[[0,1],[0,0],[1,0],[1,1]]
        # locations.append(agent_x)
        # locations.append(agent_y)
        #states.append([agent_x,agent_y,])
        #for i in range (4):
        if location == [0,0]:
            for points in required_order0:
                if points == [0,0]:
                    states.extend([1,1,1])
                elif points == location2:
                    if points == [0,0]:
                        states.extend([1,1,0])
                    elif points == [1,0]:
                        states.extend([-1,1,0])
  ...
  #############      #### FOR ROBOT 2
        if location2 == [0,0]:
            for points in required_order0:
                if points == [0,0]:
                    states2.extend([1,1,1])
                elif points == location:
                    if points == [0,0]:
                        states2.extend([1,1,0])
                    elif points == [1,0]:
                        states2.extend([-1,1,0])
                    elif points == [1,1]:
                        states2.extend([-1,-1,0])
